//
//  AutofillListTableViewCell.swift
//  MoneyManageApp
//
...
class AutofillListTableViewCell: UITableViewCell {
    ...
    func configureAutoCell(_ auto: Auto) {
        ...
        if auto.memo == "" {
            memoLabel.text = auto.category
        } else {
            memoLabel.text = auto.memo
        }
        
        if auto.payment == "支出" {
            backView.backgroundColor = UIColor(named: CARROT_ORANGE)
            priceLabel.text = "¥-" + String(result)
        } else {
            backView.backgroundColor = UIColor(named: O_BLUE)
            priceLabel.text = "¥" + String(result)
        }
        
        if auto.isInput == true {
            inputLabel.text = "今月入力済み"
            checkImageView.isHidden = false
        } else {
            inputLabel.text = "今月未入力"
