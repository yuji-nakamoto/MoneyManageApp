...
class FPension {
    ...
    class func deletePension(id: String, completion: @escaping() -> Void) {
        COLLECTION_INCOME.document(User.currentUserId()).collection("pension").document(id).delete { (error) in
            if let error = error {
