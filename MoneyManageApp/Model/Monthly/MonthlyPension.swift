...
class FMonthlyPension {
    ...
    class func fetchMPension(completion: @escaping(FMonthlyPension) -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mPension").getDocuments { (snapshot, error) in
            ...
            if snapshot?.documents == [] {
                completion(FMonthlyPension(dict: [TOTAL_PRICE: 0]))
            }
    ...
    class func deleteMPension(timestamp: String, completion: @escaping() -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mPension").document(timestamp).delete { (error) in
            if let error = error {
