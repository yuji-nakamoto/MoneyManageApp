...
class FMonthlyDating {
    ...
    class func fetchMDating(completion: @escaping(FMonthlyDating) -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mDating").getDocuments { (snapshot, error) in
            ...
            if snapshot?.documents == [] {
                completion(FMonthlyDating(dict: [TOTAL_PRICE: 0]))
            }
    ...
    class func deleteMDating(timestamp: String, completion: @escaping() -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mDating").document(timestamp).delete { (error) in
            if let error = error {
