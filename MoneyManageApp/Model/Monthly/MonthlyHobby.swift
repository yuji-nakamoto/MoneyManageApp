...
class FMonthlyHobby {
    ...
    class func fetchMHobby(completion: @escaping(FMonthlyHobby) -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mHobby").getDocuments { (snapshot, error) in
            ...
            if snapshot?.documents == [] {
                completion(FMonthlyHobby(dict: [TOTAL_PRICE: 0]))
            }
    ...
    class func deleteMHobby(timestamp: String, completion: @escaping() -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mHobby").document(timestamp).delete { (error) in
            if let error = error {
