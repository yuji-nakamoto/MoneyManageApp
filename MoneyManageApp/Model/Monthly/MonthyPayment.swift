...
class FMonthlyPayment {
    ...
    class func fetchMPayment(completion: @escaping(FMonthlyPayment) -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mPayment").getDocuments { (snapshot, error) in
            ...
            if snapshot?.documents == [] {
                completion(FMonthlyPayment(dict: [TOTAL_PRICE: 0]))
            }
    ...
    class func deleteMPayment(timestamp: String, completion: @escaping() -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mPayment").document(timestamp).delete { (error) in
            if let error = error {
