...
class FMonthlyBrush {
    ...
    class func deleteMBrush(timestamp: String, completion: @escaping() -> Void) {
        COLLECTION_MONTHLY.document(User.currentUserId()).collection("mBrush").document(timestamp).delete { (error) in
            if let error = error {
